...
Add `docs/library-authoring.md` documenting the first-class library author experience: writing normal Clojure libraries, testing pure code, requiring Atom namespaces through a selected checkout, and running daemon-world integration tests with `atom.test.alpha`.
...
- Cover:
  ...
  - `deps.edn` examples with Atom as a local-root test dependency
  - pure Clojure tests
  - author test-JVM tests requiring Atom namespaces
  - daemon-world tests using `atom.test.alpha`
  - classpath boundary between author test JVM and daemon JVM/library classloader
  ...
  - CI checkout/pinning against a selected Atom commit/tag
...
- `docs/library-authoring.md` gives an external author enough information to set up tests against a selected Atom checkout.
- The docs distinguish test-JVM `require` from daemon-side `libs/sync!`/`use!`.
- Examples use `atom.test.alpha` and do not rely on the user's default `~/.config/atom` world.
