...
- **SPEC-002.C16:** `weaver start` resolves the selected config-dir, reads `config.json`, requires valid `source`, launches the Clojure weaver from that source in the foreground, and passes the selected config-dir into the weaver even when repo discovery selected it. A discovered `.skein` without local source config fails with remediation to run `strand init --source <skein-source>` or set `SKEIN_SOURCE`. The weaver owns storage selection and loads selected config-dir `init.clj` when present.
