...
- **SPEC-002.C16:** `weaver start` asks `mill` to resolve the selected world and start that world's Clojure weaver as a mill child process using mill-derived XDG state/data dirs and mill-resolved source. It returns selected-world JSON status and does not run the weaver in the foreground terminal. If mill cannot resolve source from `SKEIN_SOURCE`, install-time embedded source, or canonical Skein checkout cwd, startup fails with remediation to provide one of those sources. The weaver owns storage selection/preparation and loads selected config-dir startup files in order: `init.clj`, then `init.local.clj`; missing files are skipped and present failing files fail startup loudly.
- **SPEC-002.C17:** `weaver repl` asks `mill` to resolve the selected world, verify that world's weaver is running, and provide the source launch context, then launches a local plain Clojure helper REPL already connected directly to the weaver nREPL endpoint. It passes explicit selected config/state metadata to the helper process and does not read source from `config.json`.
